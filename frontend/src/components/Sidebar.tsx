      ...
      <div className="flex items-center gap-3 px-3 py-4 mb-4">
        ...
        <div>
          <p className="font-headline-md text-primary text-label-lg leading-tight">
            {userDept ? `${userDept} Dept` : <BilingualText text="Department Dashboard" />}
          </p>
          <p className="text-label-sm text-on-surface-variant opacity-70"><BilingualText text="Active Session" /></p>
        </div>
      </div>
        ...
        {userRole === 'admin' && (
          <>
            <div className="pt-4 pb-2 px-4">
              <p className="text-xs font-bold text-on-surface-variant uppercase tracking-wider"><BilingualText text="Admin Controls" /></p>
            </div>
            <Link className={getLinkClass('dashboard')} to="/admin/dashboard">
              <LayoutDashboard size={20} />
              <BilingualText text="Analytics" />
            </Link>
            <Link className={getLinkClass('grievances')} to="/admin/grievances">
              <FileText size={20} />
              <BilingualText text="All Grievances" />
            </Link>
            <Link className={getLinkClass('users')} to="/admin/users">
              <Users size={20} />
              <BilingualText text="Department Users" />
            </Link>
            <a className="flex items-center gap-3 px-4 py-3 text-on-surface-variant hover:bg-surface-container-low rounded-xl font-label-lg transition-all duration-300" href="#">
              <Settings size={20} />
              <BilingualText text="Global Settings" />
            </a>
          </>
        )}
        ...
        {userRole === 'department' && (
          <>
            <Link className={getLinkClass('dashboard')} to={`/${userRole}/dashboard`}>
              <LayoutDashboard size={20} />
              <BilingualText text="Workflow Board" />
            </Link>
            <Link className={getLinkClass('map')} to={`/${userRole}/map`}>
              <MapIcon size={20} />
              <BilingualText text="Route Planning" />
            </Link>
          </>
        )}
      </nav>
